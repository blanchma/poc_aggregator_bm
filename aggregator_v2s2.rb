...
class ChangesV2
  ...
  def store_snapshot
    redis.hset "location:#{location_avid}:snapshots", Time.now.to_i, MessagePack.pack(devices)
  end

  def latest_snapshot
    snapshots = redis.hkeys "location:#{location_avid}:snapshots"
    latest_snapshot_key = snapshots.sort.last
    latest_snapshot = redis.hget "location:#{location_avid}:snapshots", latest_snapshot_key if latest_snapshot_key
    MessagePack.unpack(latest_snapshot)
  end
